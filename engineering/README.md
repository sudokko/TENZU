...
- 技術スタック: **Next.js / AWS Amplify Hosting / Stripe / Resend**（購入確認・復元マジックリンク配送。配送業者は env `MAIL_PROVIDER` で差し替え可能・SES は予備経路として残置＝[../decisions.md §3.111](../decisions.md)）**/ DynamoDB + S3**（オンサイトメッセージの定義・日次カウンタ・カード画像のみ。購入フローは引き続き DB レス・[../decisions.md §5.15](../decisions.md)）
