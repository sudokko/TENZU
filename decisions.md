...
### 5.20 運転面は Notion・設計面は repo に分ける（開店後の運用レイヤー確定）（2026-08-30）

開店（T=0）当日、オーナーから「設計書は SSOT として正しいが、**自分が見にくく・管理しにくい**」という指摘。原因を調べたところ、Notion ワークスペースに**旧計画と現行計画が併存**していた（🧭「TENZUリリース後にやること」＝2026-07 の旧 4 フェーズ版・T=0 を 11 月中旬としたまま／🗓️「TENZU マスタースケジュール & TODO」＝現行の T=0=8/30 版）。タイトル上は前者のほうが「開店後にやること」に見えるため、開くたびに撤回済みの計画へ突き当たる状態だった。

- **判断**: 情報を「**決めたこと**」と「**動いているもの**」で分け、置き場を変える。SSOT 原則（[CLAUDE.md](./CLAUDE.md) 原則④）は**同じ概念を 2 か所に書かない**ことなので、性質の違うものを別の面へ置くのは違反しない。むしろ設計書に運転状態を混ぜていたことが読みにくさの原因だった
- **repo（設計書）＝決めたこと**: 期定義・KPI 目標値・craft・判断ログ。変更頻度が低く、版管理が要る。Claude が読む
- **Notion 🗓️ ＝運転席**: TODO のチェック・週次の数値・関門の進捗。変更頻度が高く、スマホで触る。オーナーが毎週見る唯一の面
- **新設した DB 2 本**（🗓️ 配下）: ①**週次運転ログ**（週／縮退Lv／`generated_pdf`／`purchase` 紙・工房／SC クリック／B2 テーマ／B3 実施／持ち越し）＝[ops-log.md](./launch/ops-log.md) と同じ列 ②**関門・KPI ボード**（本格化ゲート T1-T3・11 月末 KPI・6 ヶ月 KPI・広告の判断＝計 15 指標に目標値と判定日を投入済み）
- **転記はしない**: 二重管理は縮退 2 の週に真っ先に死ぬ。Claude が `/weekly-ops` で repo と Notion の両方へ書く。**オーナーが手で写す運用は作らない**
- **旧 🧭 ページは退避**: タイトルへ【旧・撤回済み】を付し、冒頭に現行ページへの誘導 callout を挿入。削除はしない（判断の証跡）。生きていた運転ルール（週 3 ブロック制・チャネル優先順位・縮退表・ツールロードマップ）は 🗓️ の「運転リファレンス」へ移設した
- **Notion を計画の第 2 の正本にはしない**: 期定義・KPI の目標値を Notion 側で書き換えない。数字を動かすときは設計書を直し、Claude が Notion のプロパティへ反映する。この向きを逆流させると §5.20 以前の状態に戻る
- **未了**: ローカル Claude Code 側の Notion 連携が未確認。繋がっていなければ `/weekly-ops` の Notion 書き込みは手当てが要る（週 1 回なので当面は会話側から書いてもよい）
- 一次ソース: 本項。運転レイヤーの詳細は [launch/operations.md §9](./launch/operations.md)


### 5.21 記事の開店実績は公開 14／温存 6・週 1 公開の先頭は P2 公文（2026-08-30）

開店当日、オーナーから「記事を 10 温存とは何か／そんなに貯めていたか／今が公開しすぎではないか」という確認。実データを照合したところ、**計画（開店 10＋温存 10）と実態（公開 14＋温存 6）がずれていた**。判定は MDX frontmatter の `status:` で、無記載＝published・`status: draft` のみ非公開（`web/app/articles/articles-data.ts:81`）。

- **公開 14 本**: P1 点図形とは／C3-1 見取り図／C4-1 効果／C4-2 空間認識／P3 写す力／P4 選び方／P5 教え方／C3-2 苦手な子へ／C3-4 学術根拠／ABOUT／印刷設定／コンビニ印刷／F-1 商業利用／F-2 教員ライセンス
- **温存 6 本**: P2 公文／L-1 選び方完全ガイド／C3-3 小4つまずき／L-2 空間認知 Q&A／C3-6 図形模写との違い／C1-6 小受
- **判断①：公開しすぎとはしない**。主目的は集客＝カテゴリ資産の先置きで、SEO 本格効果は +3 ヶ月。4 本多く出したぶん 1 月の春 LP 時点で熟成が進む方向に働く。開店ゲート G3 の必須 2 本（C3-1・P1）も公開済みで、ゲートは正しく通っている
- **判断②：代償は温存在庫が 6 週分しかないこと**。9 月第 1 週から週 1 で出すと 10 月中旬で切れる（当初は 11 月第 1 週まで想定）。**継ぎ方は B2（隔週 1 本）の新規を 3 本足す**を基本線とし、回らなければ週 1 → 隔週へ落とす。10 月の月次チェックで選ぶ
- **判断③：週 1 公開の先頭は P2 公文ハブ（`kumon-math-shape`）**。Pillar 5 本で唯一未公開だった。CEP2「公文に通わせているが図形が手薄だと気づいたとき」直撃で、中核セグメント §5.1.a 先取り知育の代表的な入口。ここが欠けている間、公文文脈の受け皿が存在しない
- **反映先**: [phases.md §3.4](./launch/phases.md)（温存設計・公開順）・§4.3・§4.4 KPI・§8.1／[plan.md §5](./launch/plan.md) 死の谷対策／[channels.md §2.1](./acquisition/channels.md)／[clusters.md](./content/clusters.md)／[brand.md §11.3](./foundation/brand.md)。Notion 関門・KPI ボードの「記事公開 累計」も実態値へ更新
- 一次ソース: 本項。以後、記事の公開/温存の実態は `web/content/articles/*.mdx` の `status:` が正


### 5.22 静かな開店期の広告＝Meta/Google 各 ¥5,000・RTG は本格化から・着地は TOP と /maker（2026-08-30）

9/1 の広告開始にあたり、配分・着地・訴求をオーナー判断で確定した。旧設計（Meta ¥6,000／Google ¥3,000／RTG ¥1,000・専用 LP 前提）からの変更。

- **配分を 50/50 へ**（Meta ¥5,000／Google 検索 ¥5,000・各 約 ¥165/日）。理由＝どちらがコスパよいかを見るため。**ただし両者は同じ土俵ではない**——Google は競合性 0-4 の空き地で既知層を刈り取る／Meta は初見を掘り起こす。`generated_pdf` 単価だけで比べれば Google が勝つのはほぼ確実で、それは Meta の不良を意味しない。**判断材料は `generated_pdf` → 商品ページ遷移率**
- **リターゲティング ¥1,000 は削除**、本格化（12 月）から。開店直後はオーディエンスが空で、予算を置いても消化されない
- **新規 LP は作らない**。[funnel.md §13](./acquisition/funnel.md) の 7 セクション LP は本格化（¥30,000）で制作。¥10,000 の規模で B2 の記事 1 本分の工数を割く価値が薄い
- **着地は検索意図別に既存 2 ページ**: ①購入意図（「点描写 プリント」1,600/月・競合性 4）→ **TOP `/`**（9 種類の実演→購入の流れ→レベル選びガイドの順に既に組んである） ②無料意図（「点描写 無料」50・競合性 0）→ **`/maker`** ③Meta（潜在層）→ **`/maker` 直行**
- **Meta の着地に `/makers`（工房一覧）を採らない**（オーナー判断）。Hero とリードで説明が入るぶん初見に優しいが、`tool_start` までに 1 クリック増える。**導線の手間を減らすほうを取る**
- **訴求 3 本**: A 専門店・体系（¥200 均一・買う前に難易度と中身が見える＝[brand.md §0.5](./foundation/brand.md) クリティカルコアの直接表現）／B 無料で作れる（ブラウザで開くだけ）／C 点つなぎの次・作るのは画面、練習は紙（CEP1・3・13）
- **Meta の最適化はリンククリックで開始**。`tool_start`/`generated_pdf` は計測するが、Meta の学習には最適化イベントが週 50 件必要で ¥165/日では届かない。ピクセルにデータを溜めるのを優先し、CV 最適化は本格化から
- **実装のブロッカー＝計測の配線**。Meta ピクセルも Google 広告タグもサイトに入っていない（コードに `fbq` も `AW-` も無い）。Google 側は GA4 リンク＋キーイベント `generated_pdf` のインポートで済むので、**貼るのは Meta ピクセルだけ**。GTM のカスタムイベントトリガーはアプリが両形式（`gtag` とオブジェクト形式 `{event:...}`）を dataLayer へ積んでいるのでそのまま拾える（`web/app/analytics.ts:42`）
- **手順書は Notion「📣 広告の始め方（9/1 実装手順）」**（運転席の子ページ）。設計の正本は [ads.md §2.1 / §2.1.1](./acquisition/ads.md)
- 避ける KW＝「こぐま会 点図形」（40/月・競合性 100）。Google の自動作成アセットと Meta の Advantage+ 自動強化は **OFF**（[voice-tone.md §7.2](./foundation/voice-tone.md)）
