...
| **総 SKU＝42**（ローンチ公開ベース・拡大縮小6巻は除く） | §3.75 |
...
- 回転は計 6→**5 冊**（—/1/2/1/1）
- **総 SKU の実数は 40 巻**（data.ts 実測・scale/shrink 6 巻を除くローンチ公開ベース）。§3.75 の「42」は §3.85 の移動 2→1 巻統合が未反映のまま残っていた既存ドリフトで、本件の −1 と合わせて 42→40。**価格表（pack-commerce §14.5 の全巻制覇 ¥8,400・実収試算）とクーポン文面の「42 SKU」は未同期＝要オーナー確認の別作業**
...
- **開店ゲート G2 の分母は 42 巻のまま**＝新 3 巻は準備中（scaffold）陳列で、publish・live 化（総 45 巻化）は開店後でよい。pack-commerce §14.5 の総額表は live 化時に要更新。
...
- **開店条件**: 全 42 巻検品→publish 完了／記事 20 本修正完了（公開は一部温存＝下記）／本番 env・計測 3 イベント・送客導線(A)／[engineering/release-testing.md](./engineering/release-testing.md) の P0 全緑（P0 は本番再走含む）
...
- 全 42 巻制覇 ¥8,400（税別計算は §14.5・Stripe 手数料差引後の実収も再確認要）
