...
- tenzu.jp 取得済み・ロゴ・SNS・既存言及など埋没コスト >> 改名ベネフィット
  - ※2026-07-22 訂正: tenzu.jp は当時**未取得**（事実誤認）。埋没コスト根拠のうちドメインは無効だが、TENZU 維持判断自体は他根拠（称呼類似・指名検索敗北）で不変。ドメイン方針の現行 SSOT は §9.2
...
> 一次ソース: 本セッションログ（2026-07-22）。姉妹店の商品設計 SSOT は各店リポジトリ／設計書が立ち次第そちらへ移管

### 9.1 姉妹店 4 店構想確定（2026-07-22）

TENZU（点図）→ tsumizu（積図）→ **折図 ORIZU（仮称・折り紙展開）** → **断図 DANZU（仮称・立体切断・展開図）** の 4 店を順次開店する方針を確定。

- **店の分割軸はフォーマット（教材のかたち：点図形の紙／積み木／折り紙／切断）**。認知操作で店を分ける案（外部 AI 提案の MAWARI/TAISHO 等）は、回転・対称・かさねが TENZU の 9 タスク内部にある以上、商品構造を解体するため棄却
- 4 店は「平面⇄立体 × 見てとらえる⇄頭の中で動かす」の 2×2 を張る（**ちからの地図**）。地図はカリキュラムではなく案内板で、入口は 2 つ（予防＝点図 Lv.1 から／対処＝断図から戻り道を逆走）
- **一斉ローンチは棄却**、2-3 ヶ月間隔の順次開店。理由: ①主獲得はバズでなく検索（§3.21 と同根）②SEO は公開日から熟成が始まる ③モニターフィードバック駆動（§3.17）と逆行 ④ラダー設計（差別化の核）は人間判断が律速で 4 店同時は品質を削る
- 構想（4 店の街）は初日から全店で見せる。「一気」の演出カードは最終店開店時の全店回遊企画として温存

### 9.2 姉妹店ドメイン分離確定（2026-07-22）—「表は分離、裏は統一」

アンブレラドメイン＋ディレクトリ統一案（4ZU.jp 型）を**棄却**し、**各店独立ドメイン**（tenzu.jp / tsumizu.jp / orizu.jp / danzu.jp 等・要取得可否＋商標調査）を確定。

**統一案棄却の決定打 4 点**（オーナー事実訂正による）:

1. **非会員モデル**（§X.1 Stripe 一本化・アカウントなし）のため「共通会員・共通カート」の便益が存在しない
2. **横断購入は稀**という読み（客は「点描写 プリント」「断面図 難しい」等の個別教材 KW で来店し、その店の課題だけ解決して帰る）
3. **計測はむしろ店別に見たい**（一元化はメリットでなくノイズ）
4. **「空間認知」アンブレラは検索実体にならない**（母数小・図形とも限らない。§3.36 の「空間認知＝RTB 層・裏」の判断と整合）

**運用設計**:

- **表（ユーザー・Google から見える層）＝完全分離**: 各店独立ドメイン・独立ロゴ・descriptor。フッター「姉妹店のご案内」で同一運営を明記した相互リンク。「ちからの地図」は集客装置ではなく**各店に置く案内板**（特に断図→点図・積図への戻り道案内）
- **裏（運営層）＝完全統一**: 単一 monorepo から 4 ドメイン配信・デザインシステム・生成エンジン・Lv.1-5 縦串・F3 語彙・voice-tone・SSOT 文書体系は全店共通。**統一すべきはドメインでなく設計資産**
- **アンブレラドメインは当面取得しない**（来店動線に乗らないため）。店が増えて一覧性自体に価値が出たら再考（分離→301 統合は後から可能）
- 受容した代償: 2 店目以降の検索立ち上がりは統一案より遅い（新規ドメイン熟成×各店）。順次開店のモニター回収期間に吸収する
- トーン・客層が異なる事業（例: シニア脳トレ転用案)は最初から街の外（別ブランド・別ドメイン）。**同じ街に入る基準 = 空間認知 × 子ども × ¥200 文法の共有**

→ 関連: §3.36（TENBI 棄却・tenzu.jp 訂正注記）／[foundation/brand.md §11.3 Brand Architecture](./foundation/brand.md)

### 9.2.1 ドメイン確定 — 完全一致 .jp 3 本＋折図は仮据え置き（2026-07-23）

ドメイン取得可否を DNS ゾーン調査で確認（whois 遮断環境のため NXDOMAIN/委任有無で判定）。結果と確定方針:

| 店 | ドメイン | 状態 | 方針 |
|---|---|---|---|
| 点図 | **tenzu.jp** | 未登録 | ✅ 即取得（完全一致） |
| 積図 | **tsumizu.jp** | 未登録 | ✅ 即取得（完全一致） |
| 断図 | **danzu.jp** | 未登録 | ✅ 即取得（完全一致） |
| 折図 | orizu.jp | **登録済**（個人・2023 取得・NS 死亡の未使用保有） | 🔶 **仮据え置き**。開店直前（9-12 ヶ月先）に決定 |

**検討し棄却した「統一」案 2 つ**:

- **全店「〜屋」.jp（tenzuya.jp 等・全て空き）**: 対称性は出るが、旗艦 tenzu の完全一致を最下位・最終開店の折図のために捨てる取引。「屋」の価値は全店適用で初めて出る（1 店だけ orizuya は元の非対称に戻る）ため、旗艦の完全一致を優先し棄却
- **全店 .net / .com 統一**: どの TLD でも 4 店完全一致で揃う組み合わせが存在しない（tenzu.net/.com・danzu.com は登録済み）。かつ .net は日本の親向け有料教材の信頼シグナルとして .jp/.com に劣る。棄却
- **.zu ドメインハック（ten.zu 等）**: .zu という TLD は実在しない（ルート NXDOMAIN）。不可能

**折図ドメインの後日 3 択**（開店直前に決定）: ①orizuya.jp（折図ブランド維持・URL のみ屋号）②別の「〜図」に改名（4 番手ゆえ改名コスト最小・紋図 monzu.jp 等は空きだが別称呼衝突あり）③orizu.jp 買取打診（未使用個人保有・期待値低）

**保険取得もしない**: orizuya.jp の先行確保は行わない（オーナー判断 2026-07-23）。折図は仮のまま、選択肢を開店時に開いておく。

**シリーズの背骨は表記ブランド「X図」**（点図・積図・折図・断図）で、これはドメイン文字列に依存せず一貫。ドメインの末尾差はブランド一貫性にほぼ影響しない。

**残タスク（手元）**: ①whois.jprs.jp で 3 本の最終確認 → 空き確認後に即取得 ②J-PlatPat 称呼検索（テンズ／ツミズ／ダンズ）×16 類（印刷物）・41 類（教育）。名称衝突 Web 調査では tenzu.app（海外 OSS・分野別・descriptor で回避）、点図（視覚障害者向け触図の確立語・SEO 主語は「点描写」で実害小）を確認済み

### 9.3 断図（仮）スコープ純化 — 展開図を核から除外（2026-07-22）

断図の問題タイプ検討（本セッション・折図 6 タイプ／断図 8 タイプ案）を受けて、オーナー指摘「展開図は切断から外れる・独立ジャンルでは」を採用し、以下 4 点を確定。

1. **断図は「切る」一本の専門店に純化**。展開図（開く）は動詞・認知処理・KW 群（「展開図 問題」「サイコロ 展開図」＝学校算数 4-5 年に太い）のいずれも別ジャンルのため核から除外。二枚看板は §9.2 で確立した「専門店の純度」方針と矛盾する
2. **入口棚として「かたちを切る（平面）」（図形分割・線を引いて分ける）Lv.2-3 を新設**。「平面を切る → 立体を切る」の一直線の縦串になり、立体切断でつまずいた子の戻り道が店内に生まれる。TENZU 分解（かさねの引き算）とは操作が別で重複なし。断図の Lv 帯は 4-5 → **2-5** へ（主役の立体切断は Lv.4-5 のまま）
3. **例外として「切断線を展開図に写す」のみ断図 Lv.5 応用棚に残置**（中受切断単元の最終形＝断図の客が求めるもの。展開図は「読めれば良い」扱い・描かせない・選ばせない）
4. **展開図は 5 店目候補「開図 KAIZU（仮）」として案レベルで棚上げ**（🔶確定ではない・開店判断は未定）。ちからの地図上は折図⇄断図の橋の位置＝「ひらく」の立体版。タイプ候補: 展開図選択／組み立て予測／サイコロ面対応

**全店共通の出題原則を新設**: **白紙作図はさせない。描くのは常に印刷された図の上**（切断＝印刷済み見取り図に線を引く／折図＝点図形グリッドに描く／展開図系＝印刷済みネットへの描き込み・選択のみ）。TENZU「1 Vol. = 1 グリッド・解答グリッド印刷済み」思想の姉妹店展開。

→ 断図タイプ構成（確定 6 タイプ）: かたちを切る（平面）Lv.2-3／きりくちのかたち Lv.4-5 ★旗艦／きったあと Lv.4-5／きりくちから当てる Lv.5／つみきを切る Lv.5（積図への橋）／すいめん Lv.5（形のみ・計算なし＝算数 NG 境界の内側）
