...
### 5.20 メーカー足元は「店紹介 → SNS」の 2 段に組む（§5.19 の /maker 除外を一部撤回）（2026-08-30）

§5.19 は SNS 導線の置き場を「フッター 1 か所・メーカー本体は除外」と決め、メーカーは完了画面 `/maker-thanks` が受ける建て付けだった。実測では **PDF 到達より前に離脱する訪問のほうが多く**、完了画面は 11 本あるメーカーのうち `/maker` にしか無い。そこで 2026-08-26 に `MakerFootSns`（サイドバー足元・PDF ボタンより下）を新設して除外を一部撤回している。今回はその足元の中身を組み直す。

- **2 段構成にする（上＝店紹介／下＝SNS）**。SNS チップは「**もう知っている人**」向けの導線で、メーカーから直接入った初見の人は素通りする。その手前に「誰が作った道具か」を置いて、素通りする前に一度は目へ入れる
- **店紹介の遷移先は `/articles/tenzu-concept`（TENZUについて）**＝店と店主の自己紹介。商品一覧ではない。**道具の足元で売りに行くと「無料の道具」という前提が濁る**ので、価格も棚も書かない
- **文中で「メーカー」と呼ばない**。初見の人には通じない社内語なので「この道具」と書く（呼称そのものの SSOT は §3.50・UI ラベルは従来どおり）
- **SNS 見出しを実態に合わせて書き直す**: 旧「新しい問題や、作り方のヒント」→「**よければ、こちらでも**」＋一言「お店の様子や、店主が日々考えていることを流しています。」。**そういう発信を実際にしていない**（Instagram＝プリント紹介／note＝設計の裏側／X・Ameba＝店主の書きもの）ため、見出しのほうが嘘になっていた。「フォローする」（こちらの都合で読む側の得が無い）・「最新情報」（企業サイトの語）も採らない
- **置き位置の条件は §5.19 のまま**＝PDF ボタンより下。道具の本業（作る→刷る）を横切らせない
- **名義の但し書きはチップ直下のまま動かさない**（§5.19）。実装は `MakerFootSns`（[web/app/maker/core/chrome.tsx](./web/app/maker/core/chrome.tsx)）と `.maker-foot-shop`（[web/app/maker/maker.css](./web/app/maker/maker.css)）。SNS 側は `SnsLinks` の `heading` / `lede` を渡すだけで、フッターの「フォローする」など他の面には波及しない

### 5.21 週次運転を「週3ブロック制（曜日自由）」から「曜日固定ルーチン」へ（2026-08-30・運転開始 8/31）

開店（T=0＝8/30）の翌日から定常運転に入るタイミングで、オーナー判断により運転の単位を曜日へ固定した。正本の表は [acquisition/sns-operations.md §4](./acquisition/sns-operations.md)、運転側の記述は [launch/operations.md §4](./launch/operations.md)。

- **決めたこと**: 月＝数値・NG grep・今週テーマ・**温存記事1本を公開**／火＝IG カルーセル／水＝X 7本予約／木＝記事・note 草稿（隔週）／金＝Pinterest 週分ピン＋DM 5件／土＝コメント・DM 返信／日＝完全オフ
- **なぜ変えたか**: 旧 §4 は「曜日固定は家庭都合で1日崩れた瞬間に『もう回らない』感を生む」として、約束を**週にブロック3つだけ**（どの曜日でもよい）に置いていた。実際に運転へ入る段になっての判断は逆で、**「いつやるか」を毎週決め直すこと自体が判断コスト**であり、ルーチンの定着（＝習慣化）を妨げる。徹底したいなら曜日を先に固定するほうが強い
- **旧設計の利点は捨てない**: B1/B2/B3 は廃止せず、**曜日を束ねた「振替の単位」**として残す（B1＝月・水・土／B2＝木／B3＝火・金）。1日飛んだだけなら縮退ではなく**同じブロック内の別の日へ寄せる**（振替は週内で完結・翌週へ持ち越さない）。ブロックごと落ちる週に初めて §7 の縮退表を使う。これで「1日崩れたら全部崩れる」を防ぎつつ、平常時は曜日で走れる
- **温存記事の公開曜日＝月**に確定（[launch/phases.md §3.4](./launch/phases.md) が「毎週決まった曜日」とだけ書いて空けていた枠を埋めた）。理由は①週の頭に鮮度シグナルを出せる②その週の SNS 素材（火の IG カルーセル・水の X）が月曜時点で確定する、の2点。**木＝草稿日とは分ける**（書く日と出す日を同じにしない）
