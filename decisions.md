...
| 公文 3 記事の 1 本統合（P2 ハブ兼クラスタ・FAQ 非設置） | §3.81 |
...
### 3.81 公文 3 記事を 1 本に統合＝P2 ハブ兼クラスタ化・「よくある質問」非設置（2026-07-18）

公文関連 3 記事（P2 ハブ next-after-kumon／C2-1 kumon-shape-supplement／C2-3 kumon-math-shape）は同一文脈のカニバリで 3 本は過剰とオーナー判断。**C2-3「公文の算数で図形が出てこない理由と、家庭での補い方」`/kumon-math-shape/` をベースに 1 本へ統合**し、P2 の席をこの 1 本で兼ねる（P5＝teaching-point-drawing と同じハブ兼クラスタ方式・§3.32 の 5 Pillar 構造は維持）。
...
**記事の段階公開**（同日改定・オーナー実測レビューペース＝3 日 1 本を反映）: 開店時は 10 本（C3-1 見取り図・P1 正規ハブ必須）を公開し、温存 10 本は開店後も 3 日 1 本でレビューを継続しながら週 1 で追加公開（9 月第 1 週〜11 月第 1 週・本格化前に全 20 本が出揃う）＝「更新している感」（サイトの鮮度シグナル・LLMO/SEO クロール誘引）を意図的に作る。開店分 10 本のレビュー完了目安＝8/11（3 日×10 本・7/13 起算）。温存分は「公開の 1 週間前までにレビュー完了」ルール（週 2.3 本レビュー＞週 1 公開のため構造的に破綻しない）
