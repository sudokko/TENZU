...
- **2026-06-03 検索意図振り分け反映（[clusters.md §1.5](clusters.md)）**: 記事スラッグは情報意図のクエリのみ。取引意図（プリント／無料／簡単／難しい／立体／年齢）は**カテゴリページ・商品タグ・メーカー**側の URL が受ける（`/products/...`・`/categories/...` 系は [engineering/](../engineering/) 管轄）。本ファイルは**記事スラッグ**を定義
