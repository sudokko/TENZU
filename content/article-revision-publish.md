...
- 対象は `web/content/articles/*.mdx` の既存記事。タイトルまたは slug から一意に特定する（**公開後の正はここ**。`docs/drafts/` は初稿アーカイブで、改訂を逆同期しない）
- 開始時に Git 状態と現在ブランチを確認し、別作業の未コミット変更があれば触れずに報告する
- 本文改訂では既存の内部リンク・商品導線・必要な frontmatter を維持する
- **記事編集の既定の入口は dev の編集モード** `/atelier/articles/<slug>`（呼び出しは `/article-edit`）。オーナーは記事を見た目のまま開いてクリックした箇所を直す。Claude が改訂したときのプレビューもここで見せる（§3.5）
...
オーナーが自分で直したい依頼（「○○の記事を編集したい」「記事を直したい」）は、§3.5 の編集モードを開いて渡すのが既定。Claude が本文を書き換える依頼が §2 以降の対象になる。
...
### §3.5 編集モード（記事編集の既定の入口）

記事を直す作業は、ローカル dev の編集モードを入口にする。オーナーが自分で直すときはここで直し、Claude が改訂したときもプレビューをここで見せる（§5）。

**呼び出し**: Claude に `/article-edit <タイトルの一部 または slug>`、または「○○の記事を編集したい」と伝える。Claude が dev サーバー（`.claude/launch.json` の `web`・port 3001）を起動して編集画面を開く（手順は `.claude/skills/article-edit/SKILL.md`）。dev サーバーが動いていれば、ブラウザで `http://localhost:3001/atelier/articles` を直接開いてもよい。

1. 記事は公開ページと同じ見た目で表示される。本文の各ブロック・タイトル・キッカー・リードはクリックでその部分のソースが開く。説明文は「タイトル・説明文」、関連記事やパンくずなど画面から開けない項目は「全文ソース」で直す
2. 保存先は `web/content/articles/<slug>.mdx` だけ。改行コードと BOM は元のまま保つ。MDX や frontmatter として読めない内容、slug の変更は保存前に止まる。開いた後にファイルが別の場所で変わっていたら保存せず、読み込み直しを求める
3. 保存はファイルを書き換えるだけで、公開はしない
4. **オーナーが編集している間、Claude は同じ記事ファイルを書き換えない**。途中で Claude に直しを頼むときは、先に編集モードで保存してから頼む（未保存の文章があると、編集画面は外部の変更を取り込まない）
5. **公開**: オーナーが編集モードで仕上げて「公開して」と伝えたら、その時点をプレビュー承認として扱う。Claude は §5 の 1〜4（差分の確認・ビルド・表示確認）を行い、問題が無ければ §6 以降へ進む。§5 で直しが必要になったら、編集モードで見せ直して止まる
6. 編集モードは dev 限定（本番ビルドでは 404）。画面は `web/app/atelier/articles/`、読み書きは `web/app/api/atelier/article/` と `article-source.ts`
...
5. ローカルプレビューは編集モード（§3.5）でオーナーへ見せ、**ここで停止する**。オーナーはその場で手直しでき、手直しが入ったら 1〜4 をやり直す。
