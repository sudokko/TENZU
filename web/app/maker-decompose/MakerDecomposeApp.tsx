...
/* こたえ＝C∖B（C の辺で B に無いもの）／取り除く線＝C∩B（C の辺で B にあるもの）。
   B にしか無い辺（C に重ならない引きすぎ）は無効＝こたえにも取り除きにも出ない。 */
function diffEdges(edgesA: Edge[], edgesB: Edge[]): { answer: Edge[]; removed: Edge[] } {
  const bset = new Set(edgesB.map(edgeKey));
  const answer: Edge[] = [];
  const removed: Edge[] = [];
  for (const e of edgesA) (bset.has(edgeKey(e)) ? removed : answer).push(e);
      ...
      <div className="app-shell">
        ...
        <main className="canvas-area">
          <div className="canvas-gridbar">
            ...
            <span className="gb-label" style={{ marginLeft: "auto" }}>のこり {answerEdges.length} 本</span>
          </div>
          ...
          <div className="canvas-toolbar">
            ...
            <div className="right">
              <span className="pos-readout">正解の図 {edgesA.length} 本 / 引くもの {edgesB.length} 本</span>
