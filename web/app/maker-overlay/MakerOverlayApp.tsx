...
export default function MakerOverlayApp() {
  ...
  /* 重ね結果の本数（重なる線は同一扱いで数える・検品用） */
  const unionCount = useMemo(
    () => new Set([...edgesA, ...edgesB].map(edgeKey)).size,
    [edgesA, edgesB],
  );
  ...
  async function doExport() {
    ...
    try {
      ...
      // tenzu_overlay_yyyymmddhhmm.pdf — 上書き事故を防ぐタイムスタンプ命名
      const d = new Date();
      const p2 = (n: number) => String(n).padStart(2, "0");
      const stamp = `${d.getFullYear()}${p2(d.getMonth() + 1)}${p2(d.getDate())}${p2(d.getHours())}${p2(d.getMinutes())}`;
      doc.save(`tenzu_overlay_${stamp}.pdf`);
    } catch (err) {
  ...
  // 今の用紙・問数で実際に印刷される点の直径（mm）— buildPageSvg と同じ計算（panes=3）
      ...
      <div className="app-shell">
        ...
        <main className="canvas-area">
          <div className="canvas-gridbar">
            ...
            <span className="gb-label" style={{ marginLeft: "auto" }}>重ね {unionCount} 本</span>
          </div>
          ...
          <div className="canvas-toolbar">
            ...
            <div className="right">
              <span className="pos-readout">図形A {edgesA.length} 本 / 図形B {edgesB.length} 本</span>
          ...
          <div className="canvas-stage">
            ...
            <div className="canvas-help">
              図形A・図形B にそれぞれ線を引きます（点を 2 つクリック／同じ線をもう一度クリックで消えます）。
              右の「重ねたかたち」が仕上がり。印刷では「図形A ＋ 図形B ＝ 空欄」が一列に並び、空欄に重ねた形を描かせます。
            </div>
          </div>
        </main>
                  ...
                  return (
                    <div className={`saved-cell${p.selected ? " sel" : ""}`} key={p.id}>
                      <button className="thumb" type="button"
                        ...
                        <PaperSVG gridSize={p.gridSize} edges={[...p.edgesA, ...p.edgesB]} showLines={true} />
                      </button>
          ...
          <details className="settings-fold">
            <summary>
              ...
              <span className="sf-current">
                用紙: {paper.label} · 問数: {perPage === "auto" ? "おまかせ" : `${perPage}問/頁`} · 並び: {pairLayout === "horizontal" ? "横一列" : "縦一列"} · 名前欄: {nameField ? "あり" : "なし"}
              </span>
            </summary>
