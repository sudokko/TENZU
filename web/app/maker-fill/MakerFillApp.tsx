...
export default function MakerFillApp() {
  ...
  const edgeCountLabel = `完成図 ${edges.length} 本 / 抜く線 ${rEdges.length} 本`;
      ...
      <div className="app-shell">
        ...
        <main className="canvas-area">
          <div className="canvas-gridbar">
            ...
            <span className="gb-label" style={{ marginLeft: 16 }}>編集モード</span>
            <div className="seg" role="group" aria-label="編集モード">
              <button type="button" aria-pressed={mode === "F"}
                onClick={() => { setMode("F"); setSelected(null); }}>完成図を描く</button>
              <button type="button" aria-pressed={mode === "R"}
                onClick={() => { setMode("R"); setSelected(null); }}
                disabled={edges.length === 0}>抜く線を選ぶ</button>
            </div>
            <span className={`gb-label gb-fair${fairness.ok ? "" : " is-bad"}`}
              style={{ marginLeft: "auto" }}>{fairness.msg}</span>
          </div>
