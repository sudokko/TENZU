...
export default function MakerScaleApp() {
  ...
  const edgeCountLabel = `もとの図 ${edges.length} 本 / 拡大図（自動）`;
  const factorLabel = `×${factor} 拡大`;
  const fitLabel = edges.length === 0
    ? "最初に置く点が「始点」になります"
    : !cur.fits
      ? "⚠ 始点から拡大すると枠からはみ出します"
      : "★＝始点（ここを固定して拡大）";
      ...
      <div className="app-shell">
        ...
        <main className="canvas-area">
          <div className="canvas-gridbar">
            ...
            <span className="gb-label">倍率</span>
            <div className="seg" role="group" aria-label="倍率">
            ...
            <span className="gb-fair" data-bad={!cur.fits && edges.length > 0}
              style={{ marginLeft: "auto", color: !cur.fits && edges.length > 0 ? "#d04848" : undefined }}>
              {fitLabel}
            </span>
          </div>

          {/* 作図に効く設定（点の大きさ・一筆書き）をグリッド直下に。詳細設定（PDF 出力系）とは分離。 */}
          ...
          <div className="canvas-toolbar">
            <div className="title">
              {editingTitle}
              <span className="small">{gridSize} × {gridSize} · {factorLabel} · {paper.label}</span>
            </div>
            <div className="right">
