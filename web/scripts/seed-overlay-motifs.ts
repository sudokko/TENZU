/* overlay（かさね）3 巻へのモチーフ追加バッチ【第2弾・2026-08-05】
   （npx tsx scripts/seed-overlay-motifs.ts [--sku <sku>] [--preview <html>] [--png <png>] [--write]）

   第1弾（2026-08-01・43問→採用28）は candidates 投入済み・SEEDS は差し替えた（git 履歴に残る）。
   第2弾の目標: published 12 問の全モチーフ化に必要な残り＝lv3 +4 / lv4 +3 / lv5 +1。
   採用率を見込み lv3=6 / lv4=5 / lv5=2 を投入する。

   第1弾検品（採用28/保留15）から読んだ敗因→今回の設計則:
   - 保留の型①「A が断片」（セダンのボンネット曲線・よこロケットの開いた機体・
     きりんの背中線）→ A は必ず閉じたシルエットにする
   - 保留の型②「格子を主役に被せる」（まどべのたいよう・ぼくじょうのうま・
     おりのライオン）→ 格子・柵は主役の顔や全身を覆わない。場面の必然
     （とびこえる・ぶらさがる・たべる）で絡みを作る
   - 採用の型: A=閉じた主役＋B=場面（じめん・くき・ほのお・やま・ぼうし）
     or 整ったパターン（あみ・レール）

     overlay-lv3-vol1（4×4・ortho45）: D 14〜20 未満
     overlay-lv4-vol1（5×5・非45°必須）: D 20〜30 未満
     overlay-lv5-vol1（6×6・非45°必須・絡み3+）: D 30〜40 未満
...
/* =========================================================================
   ...
   ========================================================================= */
const SEEDS: Seed[] = [
  /* ===== overlay-lv3-vol1（4×4・ortho45・絡み0-1・D14-20）第2弾 6問 ===== */
  {
    sku: "overlay-lv3-vol1", label: "いえ（つきよのいえ）", category: "いえ",
    pathsA: ["0,1 1,0 2,1", "0,1 0,3 2,3 2,1"],          // やねとかべ（とじたいえ）
    pathsB: ["3,1 2,2 3,3", "3,1 3,3"],                  // みかづき
  },
  {
    sku: "overlay-lv3-vol1", label: "はな（あさがお）", category: "はな",
    pathsA: ["0,0 1,1 2,1 3,0", "0,0 3,0"],              // らっぱのはな
    pathsB: ["2,1 2,3", "0,3 3,3", "2,2 1,3", "2,2 3,3"], // くき・じめん・りょうのは
  },
  {
    sku: "overlay-lv3-vol1", label: "ヨット（つきとふね）", category: "ヨット",
    pathsA: ["0,2 1,3 2,3 3,2", "0,2 3,2", "3,2 3,1"],   // ふねとはたざお
    pathsB: ["2,0 1,1 2,2", "2,0 2,2"],                  // みかづき
  },
  {
    sku: "overlay-lv3-vol1", label: "ロケット（りりく）", category: "ロケット",
    pathsA: ["0,2 0,1 1,0 2,1 2,2", "0,2 2,2"],          // きたい
    pathsB: ["0,3 1,2 2,3", "0,3 3,3"],                  // ほのおとじめん
  },
  {
    sku: "overlay-lv3-vol1", label: "どうぶつのかお（ねこ）", category: "どうぶつのかお",
    pathsA: ["0,1 3,1", "0,1 0,3", "3,1 3,3", "0,3 3,3", "2,2 2,3"], // かおとはな
    pathsB: ["0,1 1,0 2,1 3,0", "3,0 3,1"],              // ギザギザのみみ
  },
  {
    sku: "overlay-lv3-vol1", label: "たいよう（ふじのあさひ）", category: "たいよう",
    pathsA: ["2,0 1,1 2,2 3,1 2,0"],                     // たいよう
    pathsB: ["0,3 2,1", "0,3 3,3", "3,3 3,2"],           // やまのしゃめんとじめん
  },

  /* ===== overlay-lv4-vol1（5×5・非45°必須・絡み0-2・D20-30）第2弾 5問 ===== */
  {
    sku: "overlay-lv4-vol1", label: "くるま（やまみちのくるま）", category: "くるま",
    pathsA: ["0,3 2,2 3,2 4,3", "0,3 4,3", "3,2 3,1"],   // しゃたい（ボンネットまで閉じる）・アンテナ
    pathsB: ["0,4 2,3 4,4", "4,4 4,3"],                  // やまみち
  },
  {
    sku: "overlay-lv4-vol1", label: "どうぶつのかお（ぼうしのくま）", category: "どうぶつのかお",
    pathsA: ["1,1 3,1", "3,1 4,2", "1,1 0,2", "0,2 2,3", "2,3 4,2"], // かおのりんかく
    pathsB: ["1,1 0,1 0,0 4,0 4,1 3,1"],                 // ぼうし（つばつき）
  },
  {
    sku: "overlay-lv4-vol1", label: "どうぶつ（きりんのしょくじ）", category: "どうぶつ",
    pathsA: ["4,3 2,0", "2,0 1,0", "3,3 4,3", "3,3 3,4", "4,3 4,4"], // くび・あたま・からだ・あし
    pathsB: ["1,1 0,2 1,3 2,2 1,1", "1,3 1,4", "0,4 4,4"], // きのは・みき・じめん
  },
  {
    sku: "overlay-lv4-vol1", label: "き（もみのきとはこ）", category: "き",
    pathsA: ["2,0 0,3", "0,3 3,3", "3,3 2,0", "2,3 2,4"], // もみのき・みき
    pathsB: ["0,4 4,4", "3,3 4,3", "3,3 3,4", "4,3 4,4"], // じめん・プレゼントばこ
  },
  {
    sku: "overlay-lv4-vol1", label: "でんしゃ（しゃだんき）", category: "でんしゃ",
    pathsA: ["1,1 4,1", "1,1 0,3", "4,1 4,3", "0,3 4,3", "2,1 2,2"], // しゃたい・まど
    pathsB: ["4,0 2,4", "1,3 1,4", "3,3 3,4", "0,4 4,4"], // しゃだんきのぼう・しゃりん・レール
  },

  /* ===== overlay-lv5-vol1（6×6・非45°必須・絡み3-10・D30-40）第2弾 2問 ===== */
  {
    sku: "overlay-lv5-vol1", label: "き（ぶらんこ）", category: "き",
    pathsA: ["3,0 1,3", "1,3 5,3", "5,3 3,0", "3,3 3,5", "0,5 5,5"], // おおきなき・みき・じめん
    pathsB: ["2,1 2,4", "4,1 4,4", "2,4 4,4", "2,1 4,1"], // ぶらんこ（わくとざせき）
  },
  {
    sku: "overlay-lv5-vol1", label: "どうぶつ（さくをとぶうま）", category: "どうぶつ",
    pathsA: ["1,2 3,2", "1,2 1,3", "3,2 3,3", "1,3 3,3", "3,2 4,0", "4,0 5,0", "2,3 0,5", "3,3 5,5"], // からだ・くび・あたま・のびたあし
    pathsB: ["0,4 5,4", "0,5 5,5", "0,4 0,5", "5,4 5,5", "2,1 2,5"], // さくのよこいたとはしら
  },
];
