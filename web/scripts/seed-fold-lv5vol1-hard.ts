...
function qLibrary(): { key: string; paths: string[] }[] {
  ...
  const frames: Record<string, string[]> = {
    わく: ["0,0 5,0", "5,0 5,5", "0,5 5,5", "0,0 0,5"],
    うえなし: ["5,0 5,5", "0,5 5,5", "0,0 0,5"],
    したなし: ["0,0 5,0", "5,0 5,5", "0,0 0,5"],
    たな: ["0,1 5,1", "0,5 5,5", "0,1 0,5", "5,1 5,5"],
    よこながわく: ["0,1 5,1", "0,4 5,4", "0,1 0,4", "5,1 5,4"],
  };
  const interiors: { key: string; paths: string[] }[] = [];
  for (const r of [1, 2, 3, 4]) interiors.push({ key: `h${r}`, paths: [`0,${r} 5,${r}`] });
  for (const c of [1, 2, 3, 4]) interiors.push({ key: `v${c}`, paths: [`${c},0 ${c},5`] });
  /* 内側の線は 1〜3 本。3 本まで許すのは D と絡みのため——2 本止まりだと
     この帯（40〜60）に届かない P が多く、探索が「該当なし」だらけになる。 */
  const combos: { key: string; paths: string[] }[][] = [];
  for (let i = 0; i < interiors.length; i++) {
    combos.push([interiors[i]]);
    for (let j = i + 1; j < interiors.length; j++) {
      combos.push([interiors[i], interiors[j]]);
      for (let k = j + 1; k < interiors.length; k++) combos.push([interiors[i], interiors[j], interiors[k]]);
    }
  }
  for (const [fk, fp] of Object.entries(frames)) {
    for (const picks of combos) {
      // たな系のわくは r=1..5 なので、r=0 を通る内側のたて線は端が浮く＝使わない
      if ((fk === "たな" || fk === "よこながわく") && picks.some((p) => p.key.startsWith("v"))) continue;
      out.push({
        key: `${fk}+${picks.map((p) => p.key).join("+")}`,
        paths: [...fp, ...picks.flatMap((p) => p.paths)],
      });
    }
  }
  return out;
...
const SEEDS: Seed[] = [
  /* P の設計則（この巻で通すための 3 条件・探索の診断から確定）
     ①非45° を 2〜3 本 … requireNon45 を満たし、E の最大レバーにもなる
     ②折り退化 r ≤ 0.34 … **左右で辺の種類を変える**（片側は垂直・片側はななめ）。
       盤面中央にそろった長いよこ線は単位辺がまるごと自分に写るので置かない
     ③盤面の c1..4 / r1..4 に収める … Q のわく（ふち）と辺を共有しない
     ①②は同じ手で片づく＝**かたむいたシルエット**にすること。 */

  /* ===== 飲み物 ×3 ===== */
  {
    label: "のみもの（たなのジュース）", category: "のみもの",
    // グラス（右がすぼまる胴）＋ななめのストロー
    pathsP: ["1,1 3,1", "1,1 1,4", "3,1 2,4", "1,4 2,4", "3,1 5,0"],
    pathsQ: [],
  },
  {
    label: "のみもの（まどべのコップ）", category: "のみもの",
    // 下すぼまりのコップ（左右で傾きがちがう）＋ゆげ
    pathsP: ["2,1 4,1", "2,1 1,3", "1,3 3,4", "3,4 4,3", "4,3 4,1", "3,1 3,0"],
    pathsQ: [],
  },
  {
    label: "のみもの（パックジュース）", category: "のみもの",
    // かたむいたパック（平行四辺形）＋ストロー
    pathsP: ["1,4 2,1", "2,1 4,1", "4,1 3,4", "3,4 1,4", "3,1 4,0"],
    pathsQ: [],
  },

  /* ===== ロボット ×3 ===== */
  {
    label: "ロボット（あしばのロボ）", category: "ロボット",
    // 台形のどう（左右でひらきがちがう）＋あたま＋あげたうで
    pathsP: ["1,1 2,1", "1,1 1,2", "2,1 2,2", "1,2 3,2", "1,2 2,4", "3,2 4,4", "2,4 4,4", "3,2 5,1"],
    pathsQ: [],
  },
  {
    label: "ロボット（はいせんのロボ）", category: "ロボット",
    // 箱のどう＋右下へのびるあし（非45°）
    pathsP: ["1,1 3,1", "1,1 2,3", "3,1 4,3", "2,3 4,3", "2,3 2,4", "4,3 4,4", "2,1 2,0"],
    pathsQ: [],
  },
  {
    label: "ロボット（キャタピラロボ）", category: "ロボット",
    // 上はんしん＋ななめ前面のキャタピラ
    pathsP: ["1,1 3,1", "1,1 1,3", "3,1 3,3", "1,3 4,4", "4,4 5,2", "5,2 3,3"],
    pathsQ: [],
  },

  /* ===== 図形 ×5 ===== */
  {
    label: "かたち（かたむいたひし）", category: "かたち",
    // 頂点をずらした四角（45°と非45°が混ざる）
    pathsP: ["2,1 1,3", "1,3 3,4", "3,4 4,3", "4,3 4,2", "4,2 2,1"],
    pathsQ: [],
  },
  {
  ...
  },
  {
    ...
    // 左を向いた矢印（先端が非45°・じくの太さが上下でちがう）
    pathsP: ["1,2 3,1", "3,1 3,2", "3,2 4,2", "4,2 4,3", "4,3 3,3", "3,3 3,4", "3,4 1,3"],
    pathsQ: [],
  },
  {
    label: "かたち（かいだんけい）", category: "かたち",
    // 段のたかさがそろわない階段（右はしがななめ）
    pathsP: ["1,4 1,2", "1,2 3,1", "3,1 3,2", "3,2 4,2", "4,2 5,4", "5,4 1,4"],
    pathsQ: [],
  },
  {
    label: "かたち（はねのついたわく）", category: "かたち",
    // 平行四辺形＋左右で長さのちがうはね
    pathsP: ["1,2 3,1", "3,1 4,2", "4,2 2,4", "2,4 1,2", "3,1 5,0"],
    pathsQ: [],
  },

  /* ===== 動物のシルエット ×3 ===== */
  {
    label: "どうぶつ（はしるいぬ）", category: "どうぶつ",
    // からだ＋あたま（右上）＋しっぽ（左下へななめ）
    pathsP: ["1,2 3,2", "3,2 4,1", "4,1 4,2", "1,2 1,4", "4,2 3,4", "1,4 3,4", "1,2 0,4"],
    pathsQ: [],
  },
  {
    label: "どうぶつ（とりのかげ）", category: "どうぶつ",
    // つばさを広げた鳥（左右で角度がちがう）
    pathsP: ["1,2 2,1", "2,1 3,2", "3,2 4,1", "4,1 4,3", "4,3 2,4", "2,4 1,2"],
    pathsQ: [],
  },
  {
    label: "どうぶつ（くじら）", category: "どうぶつ",
    // くじら（右に尾びれ）
    pathsP: ["1,3 2,1", "2,1 4,2", "4,2 4,1", "4,1 4,3", "4,3 3,4", "3,4 1,3"],
    pathsQ: [],
  },

  /* ===== 家 ×3 ===== */
  {
    label: "いえ（やまのいえ）", category: "いえ",
    // やねの左右で傾きがちがういえ＋とびら
    pathsP: ["1,3 2,1", "2,1 4,2", "4,2 3,4", "3,4 1,4", "1,4 1,3", "2,4 2,3"],
    pathsQ: [],
  },
  {
    label: "いえ（えんとつのいえ）", category: "いえ",
    // やねが非45°・えんとつは右がわ
    pathsP: ["1,3 2,1", "2,1 4,2", "1,3 1,4", "4,2 3,4", "1,4 3,4", "2,1 2,0", "2,4 2,3"],
    pathsQ: [],
  },
  {
    label: "いえ（さくのあるいえ）", category: "いえ",
    // 片流れのやねのいえ＋まど
    pathsP: ["1,4 1,2", "1,2 3,1", "3,1 4,3", "4,3 3,4", "3,4 1,4", "1,3 2,3"],
    pathsQ: [],
  },

  /* ===== 車 ×3 ===== */
  {
    label: "くるま（にもつのトラック）", category: "くるま",
    // 荷台（後ろが高い）＋運転席（まえが非45°）＋タイヤ
    pathsP: ["1,2 3,1", "1,2 1,3", "3,1 3,2", "3,2 4,2", "4,2 5,4", "1,3 5,4"],
    pathsQ: [],
  },
  {
    label: "くるま（レーシングカー）", category: "くるま",
    // ひくい車体（まえがななめ・うしろが非45°）＋タイヤ
    pathsP: ["1,3 2,2", "2,2 3,2", "3,2 4,1", "4,1 5,3", "5,3 1,4", "1,4 1,3"],
    pathsQ: [],
  },
  {
    label: "くるま（バスていのバス）", category: "くるま",
    // 前面が非45°のバス＋まど
    pathsP: ["1,1 3,1", "1,1 1,3", "3,1 4,3", "4,3 2,4", "2,4 1,3", "2,1 2,2"],
    pathsQ: [],
  },
  ...
  /* ===== 果物 ×3 ===== */
  {
    ...
    // りんご（右が急にふくらむ）＋へた
    pathsP: ["1,2 2,1", "2,1 4,2", "4,2 4,4", "4,4 2,4", "2,4 1,2", "2,1 2,0"],
    pathsQ: [],
  },
  {
    label: "くだもの（なし）", category: "くだもの",
    // 上がほそく下がふくらむ形（左右で傾きがちがう）＋え
    pathsP: ["2,1 3,1", "3,1 4,3", "4,3 3,4", "3,4 2,4", "2,4 2,2", "2,2 2,1", "3,1 4,0"],
    pathsQ: [],
  },
  {
    label: "くだもの（さくらんぼ）", category: "くだもの",
    // 大きさのちがうふたつのみ＋え
    pathsP: ["1,3 2,2", "2,2 3,3", "3,3 2,4", "2,4 1,3", "3,3 4,4", "4,4 5,2", "5,2 3,3", "2,2 2,1"],
    pathsQ: [],
  },
];
...
function evalPair(pathsP: string[], pathsQ: string[], n: (typeof FOLD_LADDER)[string]["grid"]) {
  ...
  if (r > 0.34) errs.push(`折り退化 r=${r.toFixed(2)}（折り係数 ${foldFactor(r)}）＝P が折り軸に対称ぎみ・採用実績の上限 0.33 超`);
